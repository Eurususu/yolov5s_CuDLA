...
- **Python (export tooling):** fully installed and verified — see [Pipeline A](#pipeline-a--coco-qat-training--dla-deployment) for install commands and [Troubleshooting Log](#troubleshooting-log) for the traps. `torch` is installed (see above) — only QAT fine-tuning still needs the GPU server.
