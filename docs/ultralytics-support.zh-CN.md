...
## 8. P1.5 实测结果（2026-09-07）：chunk→双 Conv 拆分，原生 v8 骨干解锁

`export_dla.py` 新增 **DLA 友好拆分**（默认启用）：把每个 C2f/C3k2 的
`cv1: Conv(c1→2c)` 沿输出通道切成 `cv1a/cv1b` 两个独立 Conv（conv 权重、融合后的
per-channel bias、以及未融合时的 BN 四参数均按半切开），forward 改为双卷积分支 ——
图里从此没有 Slice。**导出前自动做数值等价自检**（相对阈值 1e-2，实测 ~4e-4 的
卷积累加重排噪声级；注意阈值必须是相对的 —— 一个 2c 卷积与两个 c 卷积的浮点
累加顺序不同）。

| 模型 | Slice | DLA FP16 构建 | 端到端 |
|---|---|---|---|
| **yolov8s（原生骨干）** | 0 | ✅ **PASSED**（23.3MB） | ✅ 9 检测，视觉核验合理 |
| yolo11s | 0 | ❌ `/model.10/m/m.0/attn/...` **注意力算子 DLA 不支持** | — |
| yolo26s | 0 | ❌ 同上（C2PSA 注意力） | — |

v11/v26 的新堵点是 **C2PSA 注意力块**（Softmax/MatMul 类算子 standalone DLA 不支持）——
