...
# yolov5su — head lives at /model.24). Inputs produced on the training/GPU
...
#   python scripts/export_dla.py --weights qat.pt --qat --size 672 --save yolov5su_qat.onnx
#   qdq_translator.py --input_onnx_models=yolov5su_qat.onnx --infer_concat_scales --infer_mul_scales
...
CALIB_CACHE=data/loadable/qat2ptq_v5su.cache
sed "1s/^TRT-[0-9]*/TRT-${TRT_VERSION}/" data/model/yolov5su_qat_precision_config_calib.cache > ${CALIB_CACHE}
...
${TRTEXEC} --minShapes=images:1x3x672x672 --maxShapes=images:1x3x672x672 --optShapes=images:1x3x672x672 --shapes=images:1x3x672x672 \
    --onnx=data/model/yolov5su_qat_noqdq.onnx --useDLACore=0 --buildDLAStandalone \
    --saveEngine=data/loadable/yolov5su.int8.int8hwc4in.fp16chw16out.standalone.bin \
    --inputIOFormats=int8:dla_hwc4 --outputIOFormats=fp16:chw16 --int8 --fp16 \
    --calib=${CALIB_CACHE} --precisionConstraints=obey \
    --layerPrecisions="/model.24/cv2.0/cv2.0.2/Conv":fp16,"/model.24/cv2.1/cv2.1.2/Conv":fp16,"/model.24/cv2.2/cv2.2.2/Conv":fp16,"/model.24/cv3.0/cv3.0.2/Conv":fp16,"/model.24/cv3.1/cv3.1.2/Conv":fp16,"/model.24/cv3.2/cv3.2.2/Conv":fp16,"/model.24/Concat":fp16,"/model.24/Concat_1":fp16,"/model.24/Concat_2":fp16
